...
6- Run "bash /config.sh" in the real root filesystem mode at every boot to configuring the system and network. It takes more time to be completed at first run. If you see an error about network change the "linux-modules_*.deb" package corresponding to the downloaded latest Linux kernel version and CPU architecture.
